...
SCENE_LIST cSceneManager::m_eCurrScene = SLIST_LOADING;
SCENE_LIST cSceneManager::m_eBefoScene = SLIST_LOADING;
...
void cSceneManager::ChangeScene(SCENE_LIST sceneName)
{
	...
	// 새로운 씬으로 교체 및 셋업
	if (find->first != SLIST_LOADING) //로딩씬으로 전환할 때는 이전씬, 현재씬 값 변경 안함
	{
		m_eBefoScene = m_eCurrScene;
