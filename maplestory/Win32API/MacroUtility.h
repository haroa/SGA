...
{
	HFONT hFont, hTmp;
	SetTextColor(hdc, RGB(255, 172, 39));
	hFont = CreateFont(FontSize, 0, 0, 0, FW_BOLD, 0, 0, 0, 0, 0, 0, 2, 0, "도담9");
	hTmp = (HFONT)SelectObject(hdc, hFont);
	SetBkMode(hdc, TRANSPARENT);
	DrawText(hdc, TextString.data(), -1, &TxtBox, DT_SINGLELINE | DT_LEFT | DT_VCENTER);
	DeleteObject(SelectObject(hdc, hTmp));
	SetBkMode(hdc, BKMODE_LAST);
}

inline  void DrawTextBoxWhite(HDC hdc, RECT TxtBox, int FontSize, string TextString)
{
	HFONT hFont, hTmp;
...
//글자체
inline  void DrawTextBoxCenter(HDC hdc, RECT TxtBox, int FontSize, string TextString)
{
	HFONT hFont, hTmp;
...
//x = 기준x y = 기준y sizeX = 보여줄화면의크기x sizeY = 보여줄화면의크기y width = 이미지의 크기 height = 이미지의 크기
inline RECT ViewPortMake(int x, int y, int sizeX, int sizeY, int Width, int Height)
{
	RECT rt;
	rt = RectMakeCenter(x, y, sizeX, sizeY);
