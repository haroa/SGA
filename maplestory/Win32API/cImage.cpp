...
void cImage::Render(HDC hdc, int destX, int destY, int sizeX, int sizeY, int width)
{
	// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
	GdiTransparentBlt(
		hdc,					// 복사 할 장소의 DC
...
void cImage::ProgressRender(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth)
{
	if (m_isTrans)	// 배경색을 없앨 경우
	{
		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
	...
	else // 원본 이미지 그대로 출력
	{
		// BitBlt : DC간의 영역끼리 서로 고속복사를 해주는 함수
		// 메모리DC에 그려진것을 화면DC로 고속복사를 한다
		BitBlt(hdc, destX, destY, sourWidth, m_pImageInfo->nHeight,
...
void cImage::FrameRender(HDC hdc, int destX, int destY, int sourX, int sourY, int FrameWidth)
{
	if (m_isTrans)	// 배경색을 없앨 경우
	{
		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
...
void cImage::ProgressBarFrameRender(HDC hdc, int destX, int destY, int sourX, int sourY, int FrameWidth, int FrameHeight)
{
	if (m_isTrans)	// 배경색을 없앨 경우
	{
		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
		GdiTransparentBlt(
			...
			destY - m_pImageInfo->nFrameHeight / 2,					// 복사 될 좌표 시작 지점 Y
			FrameWidth,	// 복사 될 이미지의 가로 크기
			FrameHeight, // 복사 될 이미지의 세로 크기
			m_pImageInfo->hMemDC,	// 복사 할 대상 DC
			sourX * m_pImageInfo->nFrameWidth, // 현재 프레임의 시작지점 X
			sourY * m_pImageInfo->nFrameHeight,// 현재 프레임의 시작지점 Y
			FrameWidth,	// 복사 영역 가로 크기
			FrameHeight,	// 복사 영역 세로 크기
			m_transColor			// 복사 할 때 제외 할 색상(투명처리)
		);
	}
	else // 원본 이미지 그대로 출력
...
//oid cImage::FrameRender(HDC hdc, int destX, int destY, int sourX, int sourY, int sizeX, int sizeY)
//
//	if (m_isTrans)	// 배경색을 없앨 경우
//	{
//		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
...
void cImage::FrameRendersize(HDC hdc, int destX, int destY, int width, int height)
{
	if (m_isTrans)	// 배경색을 없앨 경우
	{
		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
	...
	int sourX, int sourY, int maxX, int maxY, int delay)
{
	...
	FrameRendersize(hdc, destX, destY, width, height);
}
