...
enum LOAD_KIND
{
	...
	LOAD_KIND_EMPTY_MAGENTA,		// 빈 비트맵(마젠타) 이미지
	LOAD_KIND_TRANSCOLOR,	//색깔 제외
...
public:
	...
	// 빈 비트맵(마젠타)로 초기화 및 추가
	void InitForEmpty(string key, int width, int height, bool isTrans, COLORREF transColor);
	// 이미지 - 파일로 초기화 및 추가
	...
	// 사운드 - 초기화 및 추가
	void InitForSound(string key, const char* fileName, bool bgm = false, bool loop = false);
...
public:
	...
	void LoadEmpty(string key, int width, int height, bool isTrans, COLORREF transColor);
	...
	void LoadFrameImage(string key, const char* fileName, int width, int height,
		int frameX, int frameY, int x, int y, bool isTrans = false, COLORREF transColor = RGB(0, 0, 0));
	 void LoadSound(string key, const char* fileName, bool bgm, bool loop);		// 나중에 추가
